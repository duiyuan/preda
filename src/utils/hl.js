...
function searchAnnotations(workspaceState, pattern, callback) {
  ...
  workspace.findFiles(includePattern, excludePattern, limitationForSearch).then(
    function (files) {
      ...
      for (var i = 0; i < totalFiles; i++) {
        workspace.openTextDocument(files[i]).then(
          function (file) {
...
function showOutputChannel(data) {
  ...
  if (data.length === 0) {
    window.showInformationMessage(
      "No results. (Not included file types and individual files are not searched.)"
    );
    return;
  }
